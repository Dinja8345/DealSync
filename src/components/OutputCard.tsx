...
const OutputCard = ({
  ...
  const changeStatus = async (
    ...
    setContents((prevDeals) =>
      prevDeals.map((deal) => (deal._id === _id ? { ...deal, status } : deal))
    );
    ...
    <>
      {contents.map((content) => {
        ...
        if (user) {
          ...
          const targetId = content.registrantId;
            ...
            >
              ...
              <div className="text-lg font-semibold text-gray-800 text-center pt-2">
                {isUserRegistrant
                  ...
                  : `${content.registrantId} が追加 - ${invertDealType(
                      content.format
                    )}`}
              </div>
              ...
              <div className="space-y-1 text-sm text-gray-600 border-t border-gray-200 pt-3 mt-3">
                <div className="flex justify-between">
                  ...
                  <span>¥{content.money.toLocaleString()}</span>{" "}
                  {/* 金額をカンマ区切りに */}
                </div>
                <div className="flex justify-between">
                  <span className="font-medium text-gray-700">期日:</span>
                  <span>{content.dueDate || "無期限"}</span>{" "}
                  {/* 未設定の場合の表示 */}
                </div>
